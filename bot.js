...
// ─── פקודת cap: ──────────────────────────────────────────────────────────────
bot.onText(/^cap:/i, async (tgMsg) => {
  const chatId = tgMsg.chat.id;
  try {
    const text = tgMsg.text || '';

    // מילות עזר לסינון (לא קודי מדינות)
    const stopWords = new Set([
      'de','fr','en','ar','pt','es','ru','ng',
      'flat','eng','maximum','cap','th','kw'
    ]);

    // חלץ קודי מדינות (2 אותיות בלבד)
    const rawCodes = text.match(/\b[A-Za-z]{2}\b/g) || [];

    const geos = [...new Set(rawCodes
    ...
    for (const geo of geos) {
      ...
      while (skip <= 2000) {
        ...
        for (const creative of data.creatives) {
          if (creative.geo === geo) {
            const date = creative.createdAt.split('T')[0];
            byDate[date] = (byDate[date] || 0) + 1;
          }
        }
      ...
      if (Object.keys(byDate).length === 0) {
        ...
        const lastEntry = Object.entries(byDate).sort((a,b) => b[0].localeCompare(a[0]))[0];
        const count = lastEntry[1];
        results.push(geo + ' — ' + fmtDateISO(lastEntry[0]) + ' — ' + count + ' video' + (count > 1 ? 's' : ''));
      }
    }
