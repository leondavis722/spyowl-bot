...
bot.onText(/^([A-Za-z]{2})$/, async (msg, match) => {
  const geo = match[1].toUpperCase() === 'UK' ? 'GB' : match[1].toUpperCase();
...
bot.onText(/^offer$/i, async (msg) => {
  ...
  try {
    ...
    try {
      ...
      if (awsUrl) {
        // הורד את התמונה ושלח כ-buffer (ה-AWS URL פג תוקף מהר)
        const imgResp = await axios.get(awsUrl, { responseType: 'arraybuffer', timeout: 15000 });
        const imgBuf = Buffer.from(imgResp.data);
        await bot.sendPhoto(chatId, imgBuf, { caption: '🖼️ Land Preview' });
      }
    } catch (e) {
...
// ─── פקודת cap: ──────────────────────────────────────────────────────────────
bot.onText(/^cap:/i, async (msg) => {
  const chatId = msg.chat.id;
  try {
    const text = msg.text || '';
    
    // חלץ קודי מדינות (2 אותיות בלבד) מהטקסט
    const rawCodes = text.match(/\b[A-Za-z]{2}\b/g) || [];
    
    // נרמל ומסנן — UK->GB, הסר מילות עזר
    const stopWords = new Set(['de','fr','en','ar','pt','es','uk','th','kw','ru','it','nl','pl','ro','bg','hr','hu','sk','si','cs','da','fi','sv','no','is','et','lt','lv']);
    // uk הופך ל-gb
    const geos = [...new Set(rawCodes.map(c => {
      const upper = c.toUpperCase();
      if (upper === 'UK') return 'GB';
      return upper;
    }).filter(c => {
      if (stopWords.has(c.toLowerCase())) return false;
      // בדוק שזה קוד מדינה תקין ב-SpyOwl
      return true;
    }))];

    if (geos.length === 0)
      return bot.sendMessage(chatId, '❌ No valid country codes found');

    await bot.sendMessage(chatId, `🔍 Checking ${geos.length} countries...`);

    const results = [];

    for (const geo of geos) {
      // חפש את התאריך האחרון שיצאו סרטונים במדינה
      const byDate = {};
      let skip = 0;
      ...
      while (skip <= 2000) {
        ...
        // אם מצאנו לפחות תאריך אחד — עצור
        if (Object.keys(byDate).length >= 1) break;
        if (!data.hasMore) break;
        skip += 100;
      }

      if (Object.keys(byDate).length === 0) {
        results.push(`${geo} — ❌ no videos`);
      } else {
        const lastDate = Object.entries(byDate).sort((a,b) => b[0].localeCompare(a[0]))[0];
        results.push(`${geo} — ${fmtDateISO(lastDate[0])} — ${lastDate[1]} video${lastDate[1] > 1 ? 's' : ''}`);
      }
    }

    let capMsg = '📊 *Cap Summary:*\n\n';
    capMsg += results.join('\n');
    await bot.sendMessage(chatId, capMsg, { parse_mode: 'Markdown' });
