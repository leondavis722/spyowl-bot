...
bot.onText(/^offer$/i, async (msg) => {
  ...
  const idMatch = replyText.match(/🆔 ([a-f0-9]+)/);

  if (!offerMatch || !idMatch)
    return bot.sendMessage(chatId, '❌ Reply to a video message and write "offer"');

  const creativeId = idMatch[1];
  const offerLink = offerMatch[1];

  try {
    await bot.sendMessage(chatId, '🔍 Fetching offer...');

    // תמונת preview של הלנד
    try {
      const previewResp = await axios.get(
        `${API_BASE}/s3/signed-link/${creativeId}/full_preview.webp`,
        { headers: { Cookie: SESSION_COOKIE }, responseType: 'arraybuffer', timeout: 15000 }
      );
      const previewBuf = Buffer.from(previewResp.data);
      await bot.sendPhoto(chatId, previewBuf, {}, { filename: 'preview.webp', contentType: 'image/webp' });
    } catch (e) {
